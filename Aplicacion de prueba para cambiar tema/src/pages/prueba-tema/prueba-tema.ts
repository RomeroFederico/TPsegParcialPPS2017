...
import { AlertController, NavController, NavParams, LoadingController,ToastController, PopoverController, Events, Platform } from 'ionic-angular';
...
@Component({
  selector: 'prueba-tema',
  templateUrl: 'prueba-tema.html'
})
export class PruebaTemaPage {

  tema : { colorPrincipal : string, colorSecundario : string, colorFondo : string, 
           fuente : string, fuenteSize : string, fuenteColor : string, fuenteCursiva : boolean} = null
  ...
  presentPopover(myEvent) {
    let popover = this.popoverCtrl.create(MenuTemaPage, {conTemaPersonalizado : this.tema});
    popover.present({
      ev: myEvent
    });
  }
  ...
  CrearTema()
  {
    this.navCtrl.push(PersonalizarPage, {conTemaPersonalizado : this.tema});
  }

  Salir()
  {
    let alert = this.alert.create();
    alert.setTitle('¿Desea salir de la aplicacion?');
